...
export const enum ProtocolConstants {
  ...
  KeepAliveSendTime = 50000000, // 5 seconds
}
  ...
  public acceptChunk(data: VSBuffer | null): void {
    ...
    while (this._incomingData.byteLength >= this._state.readLen) {
      ...
      if (this._state.readHead) {
        ...
        if (this._isDisposed) {
          // check if an event listener led to our disposal
          break;
        }
      }
    }
...
class ProtocolWriter {
  ...
  private _writeNowTimeout: unknown = null;
...
export class PersistentProtocol implements IMessagePassingProtocol {
  ...
  private _outgoingAckTimeout: NodeJS.Timeout | number | null;

  private _incomingMsgId: number;
  private _incomingAckId: number;
  private _incomingMsgLastTime: number;
  private _incomingAckTimeout: NodeJS.Timeout | number | null;

  private _keepAliveInterval: NodeJS.Timeout | number | null;
  ...
  private _receiveMessage(msg: ProtocolMessage): void {
    if (msg.ack > this._outgoingAckId) {
      ...
      let first = this._outgoingUnackMsg.peek();

      // Loop through the unacknowledged messages until we find one that hasn't been acknowledged
      while (first && first.id <= msg.ack) {
        // Remove the confirmed message
        this._outgoingUnackMsg.pop();

        // Peek the next message
        first = this._outgoingUnackMsg.peek();
      }
    }
