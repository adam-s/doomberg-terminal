...
function breakBetweenGraphemeBreakType(breakTypeA: GraphemeBreakType, breakTypeB: GraphemeBreakType): boolean {
  ...
  if (
    ...
    breakTypeA === GraphemeBreakType.LF
  ) {
    return true; // GB4
  }
  if (
    ...
    breakTypeB === GraphemeBreakType.LF
  ) {
    return true; // GB5
  }
  ...
  if (breakTypeA === GraphemeBreakType.L) {
    if (
      breakTypeB === GraphemeBreakType.L ||
      breakTypeB === GraphemeBreakType.V ||
      breakTypeB === GraphemeBreakType.LV ||
      breakTypeB === GraphemeBreakType.LVT
    ) {
      return false; // GB6
    }
  }
...
function getOffsetBeforeLastEmojiComponent(initialOffset: number, str: string): number | undefined {
  ...
  while (
    ...
    codePoint === CodePoint.emojiVariantSelector ||
...
export class AmbiguousCharacters {
  private static readonly ambiguousCharacterData = new Lazy<
    Record<string | '_common' | '_default', /* code point -> ascii code point */ number[]>
  >(() => {
  ...
  private static readonly cache = new LRUCachedFunction<string[], AmbiguousCharacters>(
    ...
    locales => {
      ...
      function intersectMaps(map1: Map<number, number> | undefined, map2: Map<number, number>) {
        if (!map1) {
          return map2;
        }
        const result = new Map<number, number>();
  ...
  private constructor(private readonly confusableDictionary: Map<number, number>) {}
