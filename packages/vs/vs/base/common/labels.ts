...
export function tildify(path: string, userHome: string, os = OS): string {
  ...
  let normalizedUserHome =
    normalizedUserHomeCached.original === userHome ? normalizedUserHomeCached.normalized : undefined;
        ...
        for (let otherPathIndex = 0; !match && otherPathIndex < paths.length; otherPathIndex++) {
          ...
          if (otherPathIndex !== pathIndex && paths[otherPathIndex] && paths[otherPathIndex].indexOf(subpath) > -1) {
            ...
            const subpathWithSep: string =
              start > 0 && paths[otherPathIndex].indexOf(pathSeparator) > -1 ? pathSeparator + subpath : subpath;
    ...
    .filter((segment, index) => {
      ...
      if (segment.type === Type.SEPARATOR) {
        ...
        return [left, right].every(
          segment =>
            segment && (segment.type === Type.VARIABLE || segment.type === Type.TEXT) && segment.value.length > 0,
        );
      }
