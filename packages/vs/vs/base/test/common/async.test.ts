...
describe('Async', () => {
  ...
  describe('cancelablePromise', function () {
    it("set token, don't wait for inner promise", function () {
      let canceled = 0;
    ...
    it('cancel despite inner promise being resolved', function () {
      let canceled = 0;
      const promise = async.createCancelablePromise(token => {
        store.add(
          token.onCancellationRequested(_ => {
    ...
    it('execution order (sync)', function () {
      const order: string[] = [];
    ...
    it('execution order (async)', function () {
      const order: string[] = [];
    ...
    it('execution order (async with late listener)', async function () {
      const order: string[] = [];
    ...
    it('get inner result', async function () {
      const promise = async.createCancelablePromise(token => {
  ...
  describe('Throttler', function () {
    ...
    it('last factory should be the one getting called', function () {
    ...
    it('disposal after queueing', async () => {
      let factoryCalls = 0;
      const factory = async () => {
        factoryCalls++;
    ...
    it('disposal before queueing', async () => {
      let factoryCalls = 0;
      const factory = async () => {
        factoryCalls++;
        return async.timeout(0);
  ...
  describe('Delayer', function () {
    ...
    describe('ThrottledDelayer', () => {
      it('promise should resolve if disposed', async () => {
      ...
      it('trigger after dispose throws', async () => {
        const throttledDelayer = new async.ThrottledDelayer<void>(100);
        throttledDelayer.dispose();

        try {
          await throttledDelayer.trigger(async () => {}, 0);
          assert.fail('Expected trigger to throw an error after dispose');
        } catch (e) {
          assert.instanceOf(e, Error);
          assert.match((e as Error).message, /disposed/i);
        }
      });
    });

    it('simple cancel', function () {
      let count = 0;
      const factory = () => {
        return Promise.resolve(++count);
      };
    ...
    it('simple cancel microtask', function () {
      let count = 0;
      const factory = () => {
        return Promise.resolve(++count);
      };
    ...
    it('cancel should cancel all calls to trigger', function () {
      let count = 0;
      const factory = () => {
        return Promise.resolve(++count);
      };
    ...
    it('trigger, cancel, then trigger again', function () {
      let count = 0;
      const factory = () => {
        return Promise.resolve(++count);
      };
    ...
    it('last task should be the one getting called', function () {
      const factoryFactory = (n: number) => () => {
  ...
  describe('sequence', () => {
    it('simple', () => {
      ...
      return async
        .sequence([factoryFactory(1), factoryFactory(2), factoryFactory(3), factoryFactory(4), factoryFactory(5)])
        .then(result => {
  ...
  describe('Limiter', () => {
    it('assert degree of paralellism', function () {
  ...
  describe('Queue', () => {
    ...
    it('stop processing on dispose', async function () {
      const queue = new async.Queue();
    ...
    it('stop on clear', async function () {
      const queue = new async.Queue();
    ...
    it('clear and drain (1)', async function () {
      const queue = new async.Queue();
    ...
    it('clear and drain (2)', async function () {
      const queue = new async.Queue();
    ...
    it('drain timing', async function () {
      const queue = new async.Queue();
    ...
    it('drain event is send only once', async function () {
      const queue = new async.Queue();
    ...
    it('errors bubble individually but not cause stop', function () {
      const queue = new async.Queue();
    ...
    it('order is kept (chained)', function () {
      const queue = new async.Queue();
  ...
  describe('TaskSequentializer', () => {
    it('execution basics', async function () {
    ...
    it('executing and queued (finishes instantly)', async function () {
      const sequentializer = new async.TaskSequentializer();
    ...
    it('executing and queued (finishes after timeout)', async function () {
      const sequentializer = new async.TaskSequentializer();
    ...
    it('join (without executing or queued)', async function () {
      const sequentializer = new async.TaskSequentializer();
    ...
    it('join (without queued)', async function () {
      const sequentializer = new async.TaskSequentializer();
    ...
    it('join (with executing and queued)', async function () {
      const sequentializer = new async.TaskSequentializer();
    ...
    it('executing and multiple queued (last one wins)', async function () {
      const sequentializer = new async.TaskSequentializer();
    ...
    it('cancel executing', async function () {
      const sequentializer = new async.TaskSequentializer();
  ...
  describe('disposableTimeout', () => {
    ...
    it('store managed success', async () => {
      let cb = false;
    ...
    it('store managed cancel via disposable', async () => {
      let cb = false;
      const s = new DisposableStore();
    ...
    it('store managed cancel via store', async () => {
      let cb = false;
      const s = new DisposableStore();
      async.disposableTimeout(() => (cb = true), 0, s);
      s.dispose();
  ...
  describe('Promises.settled', () => {
    ...
    it('rejects with first error but handles all promises (all errors)', async () => {
    ...
    it('rejects with first error but handles all promises (1 error)', async () => {
      const p1 = Promise.resolve(1);
  ...
  describe('ThrottledWorker', () => {
    ...
    it('basics', async () => {
      ...
      const worker = store.add(
        new async.ThrottledWorker<number>(
          {
            maxWorkChunkSize: 5,
    ...
    it('do not accept too much work', async () => {
    ...
    it('do not accept too much work (account for max chunk size', async () => {
      const handled: number[] = [];
      const handler = (units: readonly number[]) => handled.push(...units);
  ...
  describe('LimitedQueue', () => {
    it('basics (with long running task)', async () => {
    ...
    it('basics (with sync running task)', async () => {
      const limitedQueue = new async.LimitedQueue();
  ...
  describe('AsyncIterableObject', function () {
    it('onReturn NOT called', async function () {
      let calledOnReturn = false;
      const iter = new async.AsyncIterableObject<number>(
        writer => {
    ...
    it('onReturn called on break', async function () {
      let calledOnReturn = false;
      const iter = new async.AsyncIterableObject<number>(
        writer => {
          writer.emitMany([1, 2, 3, 4, 5]);
    ...
    it('onReturn called on return', async function () {
      let calledOnReturn = false;
      const iter = new async.AsyncIterableObject<number>(
        writer => {
          writer.emitMany([1, 2, 3, 4, 5]);
    ...
    it('onReturn called on throwing', async function () {
      let calledOnReturn = false;
      const iter = new async.AsyncIterableObject<number>(
        writer => {
          writer.emitMany([1, 2, 3, 4, 5]);
  ...
  describe('AsyncIterableSource', function () {
    it('onReturn is wired up', async function () {
      let calledOnReturn = false;
    ...
    it('onReturn is wired up 2', async function () {
      let calledOnReturn = false;
      const source = new async.AsyncIterableSource<number>(() => {
        calledOnReturn = true;
      });
