...
describe('Glob', () => {
  ...
  it('expression with non-trivia glob (issue 144458)', function () {
    const pattern = '**/p*';
  ...
  it('expression with other falsy value', function () {
    const expr: { string: boolean } = { '**/*.js': 0 } as any;

    // Since the match function is expected to return the matching pattern (string)
    const result = glob.match(expr, 'foo.js');
  ...
  function testOptimizationForBasenames(
    ...
    matches.forEach(([text, result], i) => {
      assert.strictEqual<string | Promise<string | null> | null | boolean>(parsed(text, null!, siblingsFns[i]), result);
    });
  }
  ...
  it('expression/pattern path', function () {
    ...
    assert.strictEqual(
      glob.parse('**/foo/bar/**', { trimForExclusions: true })(nativeSep('bar/foo/bar/baz'), 'baz'),
      false,
    );
  ...
  it('relative pattern - single star alone', function () {
    if (isWindows) {
  ...
  it('relative pattern - ignores case on macOS/Windows', function () {
    if (isWindows) {
      const p: glob.IRelativePattern = { base: 'C:\\DNXConsoleApp\\foo', pattern: 'something/*.cs' };
  ...
  it('relative pattern - trailing slash / backslash (#162498)', function () {
    if (isWindows) {
  ...
  it('expression fails when siblings use promises (https://github.com/microsoft/vscode/issues/146294)', async function () {
