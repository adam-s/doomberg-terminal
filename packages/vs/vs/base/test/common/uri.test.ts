...
describe('URI', () => {
  ...
  it('URI#fsPath - no `fsPath` when no `path`', () => {
  ...
  it('http#toString', () => {
    assert.strictEqual(
      URI.from({ scheme: 'http', authority: 'www.example.com', path: '/my/path' }).toString(),
    ...
    assert.strictEqual(
      URI.from({ scheme: 'http', authority: 'example.com', path: '/', query: 'test=true' }).toString(),
      'http://example.com/?test%3Dtrue',
    );
    assert.strictEqual(
      URI.from({ scheme: 'http', authority: 'example.com', path: '/', query: '', fragment: 'test=true' }).toString(),
      'http://example.com/#test%3Dtrue',
    );
  });

  it('http#toString, encode=FALSE', () => {
    assert.strictEqual(
      URI.from({ scheme: 'http', authority: 'example.com', path: '/', query: 'test=true' }).toString(true),
      'http://example.com/?test=true',
    );
    assert.strictEqual(
      URI.from({ scheme: 'http', authority: 'example.com', path: '/', query: '', fragment: 'test=true' }).toString(
    ...
    assert.strictEqual(
      URI.from({ scheme: 'http', path: '/api/files/test.me', query: 't=1234' }).toString(true),
  ...
  it('with, changes', () => {
    ...
    assert.strictEqual(
      URI.from({ scheme: 's' }).with({ scheme: 'http', path: '/api/files/test.me', query: 't=1234' }).toString(),
      'http:/api/files/test.me?t%3D1234',
    );
    assert.strictEqual(
      URI.from({ scheme: 's' })
        .with({ scheme: 'http', authority: '', path: '/api/files/test.me', query: 't=1234', fragment: '' })
        .toString(),
      'http:/api/files/test.me?t%3D1234',
    );
    assert.strictEqual(
      URI.from({ scheme: 's' })
        .with({ scheme: 'https', authority: '', path: '/api/files/test.me', query: 't=1234', fragment: '' })
        .toString(),
    ...
    assert.strictEqual(
      URI.from({ scheme: 's' })
        .with({ scheme: 'boo', authority: '', path: '/api/files/test.me', query: 't=1234', fragment: '' })
        .toString(),
      'boo:/api/files/test.me?t%3D1234',
    );
  });

  it('with, remove components #8465', () => {
  ...
  it('parse, disallow //path when no authority', () => {
    assert.throws(() => URI.parse('file:////shares/files/p.cs'));
  });

  it('URI#file, win-speciale', () => {
    if (isWindows) {
  ...
  it('URI#file, no path-is-uri check', () => {
  ...
  it('URI#toString, escape all the bits', () => {
  ...
  it("URI#toString, don't encode port", () => {
  ...
  it('URI#toString, user information in authority', () => {
  ...
  it('URI - http, query & toString', function () {
  ...
  it('class URI cannot represent relative file paths #34449', function () {
    let path = '/foo/bar';
  ...
  it('Ctrl click to follow hash query param url gets urlencoded #49628', function () {
  ...
  it("Unable to open '%A0.txt': URI malformed #76506", function () {
  ...
  it('Bug in URI.isUri() that fails `thing` type comparison #114971', function () {
  ...
  it('from, from(strict), revive', function () {
  ...
  it("Unable to open '%A0.txt': URI malformed #76506, part 2", function () {
  ...
  it.skip('Links in markdown are broken if url contains encoded parameters #79474', function () {
  ...
  it.skip('Uri#parse can break path-component #45515', function () {
    const strIn =
  ...
  (isWindows ? it.skip : it)('URI#joinPath (posix)', function () {
  ...
  (!isWindows ? it.skip : it)('URI#joinPath (windows)', function () {
  ...
  it('vscode-uri: URI.toString() wrongly encode IPv6 literals #154048', function () {
    assert.strictEqual(
    ...
    assert.strictEqual(
      URI.parse('http://user@[FEDC:BA98:7654:3210:FEDC:BA98:7654:3210]:80/index.html').toString(),
      'http://user@[fedc:ba98:7654:3210:fedc:ba98:7654:3210]:80/index.html',
    );
    assert.strictEqual(
      URI.parse('http://us[er@[FEDC:BA98:7654:3210:FEDC:BA98:7654:3210]:80/index.html').toString(),
