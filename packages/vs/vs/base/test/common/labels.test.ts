...
describe('Labels', () => {
  (!isWindows ? it.skip : it)('shorten - windows', () => {
    // nothing to shorten
  ...
  (isWindows ? it.skip : it)('shorten - not windows', () => {
    // nothing to shorten
    assert.deepStrictEqual(labels.shorten(['a']), ['a']);
    assert.deepStrictEqual(labels.shorten(['a', 'b']), ['a', 'b']);
    assert.deepStrictEqual(labels.shorten(['/a', '/b']), ['/a', '/b']);
  ...
  it('template', () => {
    ...
    assert.strictEqual(
      labels.template('Foo ${one} Bar ${two}', { one: 'value', two: 'other value' }),
    ...
    assert.strictEqual(
      labels.template('${separator}Foo${separator}Bar', { value: 'something', separator: { label: ' - ' } }),
      'Foo - Bar',
    );
    assert.strictEqual(
      labels.template('${value} Foo${separator}Bar', { value: 'something', separator: { label: ' - ' } }),
      'something Foo - Bar',
    );
    ...
    assert.strictEqual(
      labels.template(t, { activeEditorShort: '', rootName: 'root', separator: { label: ' - ' } }),
      'root',
    );
    assert.strictEqual(
      labels.template(t, { activeEditorShort: 'markdown.txt', rootName: 'root', separator: { label: ' - ' } }),
      'markdown.txt - root',
    );
    ...
    assert.strictEqual(
      labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: '', separator: { label: ' - ' } }),
      '',
    );
    assert.strictEqual(
      labels.template(t, {
  ...
  it('getPathLabel', () => {
    ...
    assert.strictEqual(
      labels.getPathLabel(nixFileUri, { os: OperatingSystem.Windows, tildify: { userHome: nixUserHome } }),
      '\\some\\folder\\file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixFileUri, { os: OperatingSystem.Macintosh, tildify: { userHome: nixUserHome } }),
      '~/folder/file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixBadFileUri, { os: OperatingSystem.Macintosh, tildify: { userHome: nixUserHome } }),
      '/some/folder/file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixFileUri, { os: OperatingSystem.Linux, tildify: { userHome: nixUserHome } }),
      '~/folder/file.txt',
    );

    assert.strictEqual(
      labels.getPathLabel(nixFileUri, { os: OperatingSystem.Windows, tildify: { userHome: remoteUserHome } }),
      '\\some\\folder\\file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixFileUri, { os: OperatingSystem.Macintosh, tildify: { userHome: remoteUserHome } }),
      '~/folder/file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixFileUri, { os: OperatingSystem.Linux, tildify: { userHome: remoteUserHome } }),
      '~/folder/file.txt',
    );
    ...
    assert.strictEqual(
      labels.getPathLabel(nixUntitledUri, { os: OperatingSystem.Windows, tildify: { userHome: nixUserHome } }),
      '\\some\\folder\\file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixUntitledUri, { os: OperatingSystem.Macintosh, tildify: { userHome: nixUserHome } }),
      '~/folder/file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixUntitledUri, { os: OperatingSystem.Linux, tildify: { userHome: nixUserHome } }),
      '~/folder/file.txt',
    );

    assert.strictEqual(
      labels.getPathLabel(nixUntitledUri, { os: OperatingSystem.Windows, tildify: { userHome: remoteUserHome } }),
      '\\some\\folder\\file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixUntitledUri, { os: OperatingSystem.Macintosh, tildify: { userHome: remoteUserHome } }),
      '~/folder/file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixUntitledUri, { os: OperatingSystem.Linux, tildify: { userHome: remoteUserHome } }),
      '~/folder/file.txt',
    );
    ...
    assert.strictEqual(
      labels.getPathLabel(winFileUri, { os: OperatingSystem.Windows, relative: winRelativePathProvider }),
      'folder\\file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(winFileUri, { os: OperatingSystem.Macintosh, relative: winRelativePathProvider }),
      'folder/file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(winFileUri, { os: OperatingSystem.Linux, relative: winRelativePathProvider }),
      'folder/file.txt',
    );
    ...
    assert.strictEqual(
      labels.getPathLabel(nixFileUri, { os: OperatingSystem.Windows, relative: nixRelativePathProvider }),
      'folder\\file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixFileUri, { os: OperatingSystem.Macintosh, relative: nixRelativePathProvider }),
      'folder/file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixFileUri, { os: OperatingSystem.Linux, relative: nixRelativePathProvider }),
      'folder/file.txt',
    );

    assert.strictEqual(
      labels.getPathLabel(nixUntitledUri, { os: OperatingSystem.Windows, relative: nixRelativePathProvider }),
      'folder\\file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixUntitledUri, { os: OperatingSystem.Macintosh, relative: nixRelativePathProvider }),
      'folder/file.txt',
    );
    assert.strictEqual(
      labels.getPathLabel(nixUntitledUri, { os: OperatingSystem.Linux, relative: nixRelativePathProvider }),
      'folder/file.txt',
    );
  });
