...
describe('TF-IDF Calculator', function () {
  ensureNoDisposablesAreLeakedInTestSuite();
  it('Should return no scores when no documents are given', () => {
    const tfidf = new TfIdfCalculator();
    const scores = tfidf.calculateScores('something', CancellationToken.None);
    assertScoreOrdersEqual(scores, []);
  });

  it('Should return no scores for term not in document', () => {
  ...
  it('Should return scores for document with exact match', () => {
    for (const docs of permutate([makeDocument('A', 'cat dog cat'), makeDocument('B', 'cat fish')])) {
  ...
  it('Should return document with more matches first', () => {
  ...
  it('Should return document with more matches first when term appears in all documents', () => {
    for (const docs of permutate([
  ...
  it('Should weigh less common term higher', () => {
    for (const docs of permutate([
      makeDocument('/A', 'cat dog cat'),
      makeDocument('/B', 'fish'),
  ...
  it('Should weigh chunks with less common terms higher', () => {
    for (const docs of permutate([
  ...
  it('Should match on camelCase words', () => {
    for (const docs of permutate([
  ...
  it('Should not match document after delete', () => {
