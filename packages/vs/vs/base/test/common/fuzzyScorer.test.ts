...
describe('Fuzzy Scorer', () => {
  ...
  it('scoreItem - matches are proper', function () {
  ...
  it('scoreItem - multiple', function () {
    const resource = URI.file('/xyz/some/path/someFile123.txt');

    const res1 = scoreItem(resource, 'xyz some', true, ResourceAccessor);
    assert.ok(res1.score);
    assert.strictEqual(res1.labelMatch?.length ?? 0, 1);
    assert.strictEqual(res1.labelMatch?.[0].start ?? -1, 0);
    assert.strictEqual(res1.labelMatch?.[0].end ?? -1, 4);
    assert.strictEqual(res1.descriptionMatch?.length ?? 0, 1);
    assert.strictEqual(res1.descriptionMatch?.[0].start ?? -1, 1);
    assert.strictEqual(res1.descriptionMatch?.[0].end ?? -1, 4);

    const res2 = scoreItem(resource, 'some xyz', true, ResourceAccessor);
    assert.ok(res2.score);
    assert.strictEqual(res1.score, res2.score);
    assert.strictEqual(res2.labelMatch?.length ?? 0, 1);
    assert.strictEqual(res2.labelMatch?.[0].start ?? -1, 0);
    assert.strictEqual(res2.labelMatch?.[0].end ?? -1, 4);
    assert.strictEqual(res2.descriptionMatch?.length ?? 0, 1);
    assert.strictEqual(res2.descriptionMatch?.[0].start ?? -1, 1);
    assert.strictEqual(res2.descriptionMatch?.[0].end ?? -1, 4);
    ...
    assert.strictEqual(res3.labelMatch?.length ?? 0, 1);
    assert.strictEqual(res3.labelMatch?.[0].start ?? -1, 0);
    assert.strictEqual(res3.labelMatch?.[0].end ?? -1, 11);
    assert.strictEqual(res3.descriptionMatch?.length ?? 0, 1);
    assert.strictEqual(res3.descriptionMatch?.[0].start ?? -1, 1);
    assert.strictEqual(res3.descriptionMatch?.[0].end ?? -1, 4);

    const res4 = scoreItem(resource, 'path z y', true, ResourceAccessor);
    assert.ok(res4.score);
    assert.ok(res4.score < res2.score);
    assert.strictEqual(res4.labelMatch?.length ?? 0, 0);
    assert.strictEqual(res4.descriptionMatch?.length ?? 0, 2);
    assert.strictEqual(res4.descriptionMatch?.[0].start ?? -1, 2);
    assert.strictEqual(res4.descriptionMatch?.[0].end ?? -1, 4);
    assert.strictEqual(res4.descriptionMatch?.[1].start ?? -1, 10);
    assert.strictEqual(res4.descriptionMatch?.[1].end ?? -1, 14);
  });

  it('scoreItem - multiple with cache yields different results', function () {
    const resource = URI.file('/xyz/some/path/someFile123.txt');
    const cache = {};
  ...
  it('scoreItem - invalid input', function () {
  ...
  it('scoreItem - optimize for file paths', function () {
    ...
    assert.ok(pathRes.descriptionMatch);
    assert.ok(pathRes.labelMatch);
    assert.strictEqual(pathRes.labelMatch?.length ?? 0, 1);
    assert.strictEqual(pathRes.labelMatch?.[0].start ?? -1, 0);
    assert.strictEqual(pathRes.labelMatch?.[0].end ?? -1, 7);
    assert.strictEqual(pathRes.descriptionMatch?.length ?? 0, 1);
    assert.strictEqual(pathRes.descriptionMatch?.[0].start ?? -1, 23);
    assert.strictEqual(pathRes.descriptionMatch?.[0].end ?? -1, 26);
  });

  it('scoreItem - avoid match scattering (bug #36119)', function () {
    ...
    assert.strictEqual(pathRes.labelMatch?.length ?? 0, 1);
    assert.strictEqual(pathRes.labelMatch?.[0].start ?? -1, 0);
    assert.strictEqual(pathRes.labelMatch?.[0].end ?? -1, 9);
  });

  it('scoreItem - prefers more compact matches', function () {
  ...
  it('scoreItem - proper target offset', function () {
  ...
  it('scoreItem - proper target offset #2', function () {
  ...
  it('scoreItem - proper target offset #3', function () {
  ...
  it('scoreItem - no match unless query contained in sequence', function () {
  ...
  it('scoreItem - match if using slash or backslash (local, remote resource)', function () {
  ...
  it('scoreItem - ensure upper case bonus only applies on non-consecutive matches (bug #134723)', function () {
  ...
  it('compareItemsByScore - identity', function () {
  ...
  it('compareFilesByScore - basename prefix', function () {
    const resourceA = URI.file('/some/path/fileA.txt');
    const resourceB = URI.file('/some/path/other/fileB.txt');
    const resourceC = URI.file('/unrelated/some/path/other/fileC.txt');
  ...
  it('compareFilesByScore - basename camelcase', function () {
    const resourceA = URI.file('/some/path/fileA.txt');
    const resourceB = URI.file('/some/path/other/fileB.txt');
    const resourceC = URI.file('/unrelated/some/path/other/fileC.txt');
  ...
  it('compareFilesByScore - basename scores', function () {
    const resourceA = URI.file('/some/path/fileA.txt');
    const resourceB = URI.file('/some/path/other/fileB.txt');
    const resourceC = URI.file('/unrelated/some/path/other/fileC.txt');
  ...
  it('compareFilesByScore - path scores', function () {
    const resourceA = URI.file('/some/path/fileA.txt');
    const resourceB = URI.file('/some/path/other/fileB.txt');
    const resourceC = URI.file('/unrelated/some/path/other/fileC.txt');
  ...
  it('compareFilesByScore - prefer shorter basenames', function () {
    const resourceA = URI.file('/some/path/fileA.txt');
  ...
  it('compareFilesByScore - prefer shorter basenames (match on basename)', function () {
    const resourceA = URI.file('/some/path/fileA.txt');
    const resourceB = URI.file('/some/path/other/fileBLonger.txt');
    const resourceC = URI.file('/unrelated/the/path/other/fileC.txt');
  ...
  it('compareFilesByScore - prefer shorter paths', function () {
    const resourceA = URI.file('/some/path/fileA.txt');
    const resourceB = URI.file('/some/path/other/fileB.txt');
    const resourceC = URI.file('/unrelated/some/path/other/fileC.txt');
  ...
  it('compareFilesByScore - prefer shorter paths (bug #17443)', function () {
  ...
  it('compareFilesByScore - prefer matches in label over description if scores are otherwise equal', function () {
  ...
  it('compareFilesByScore - prefer camel case matches', function () {
  ...
  it('compareFilesByScore - prefer more compact camel case matches', function () {
  ...
  it('compareFilesByScore - prefer more compact matches (label)', function () {
  ...
  it('compareFilesByScore - prefer more compact matches (path)', function () {
  ...
  it('compareFilesByScore - prefer more compact matches (label and path)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #34210)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #21019 1.)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #21019 2.)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #26649)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #33247)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #33247 comment)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #36166)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #32918)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #14879)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #14727 1)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #14727 2)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #18381)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #35572)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #36810)', function () {
  ...
  it('compareFilesByScore - prefer shorter hit (bug #20546)', function () {
  ...
  it('compareFilesByScore - avoid match scattering (bug #12095)', function () {
  ...
  it('compareFilesByScore - prefer case match (bug #96122)', function () {
  ...
  it('compareFilesByScore - prefer shorter match (bug #103052) - foo bar', function () {
  ...
  it('compareFilesByScore - prefer shorter match (bug #103052) - payment model', function () {
  ...
  it('compareFilesByScore - prefer shorter match (bug #103052) - color', function () {
  ...
  it('compareFilesByScore - prefer strict case prefix', function () {
    const resourceA = URI.file('app/constants/color.js');
  ...
  it('compareFilesByScore - prefer prefix (bug #103052)', function () {
  ...
  it('compareFilesByScore - boost better prefix match if multiple queries are used', function () {
  ...
  it('compareFilesByScore - boost shorter prefix match if multiple queries are used', function () {
  ...
  it('compareFilesByScore - boost shorter prefix match if multiple queries are used (#99171)', function () {
  ...
  it('fuzzyScore2 (matching)', function () {
    const target = 'HelLo-World';
  ...
  it('fuzzyScore2 (multiple queries)', function () {
    const target = 'HelLo-World';
  ...
  it('Using quotes should expect contiguous matches match', function () {
  ...
  it('Using quotes should highlight contiguous indexes', function () {
