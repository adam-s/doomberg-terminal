...
export function deserialize(reader: IReader): any {
  ...
  switch (type) {
    ...
    case DataType.String: {
      const stringData = reader.read(readIntVQL(reader)).toString();
      return stringData;
    }
    case DataType.Buffer: {
      const bufferData = reader.read(readIntVQL(reader)).buffer;
      return bufferData;
    }
    case DataType.VSBuffer: {
      const vsBufferData = reader.read(readIntVQL(reader));
      return vsBufferData;
    }
    case DataType.Array: {
    ...
    case DataType.Object: {
      const objectData = JSON.parse(reader.read(readIntVQL(reader)).toString());
      return objectData;
    }
    case DataType.Int: {
      const intData = readIntVQL(reader);
      return intData;
    }
    default:
      return undefined;
          ...
          onFirstMessage(msg => {
            ...
            this.disposables.add(
              onDidClientDisconnect(() => {
                console.log('disconnection event', connection.ctx);
    ...
    for (const key in handler) {
      if (propertyIsEvent(key)) {
        mapEventNameToEvent.set(
          key,
          Event.buffer(handler[key] as Event<unknown>, true, undefined, disposables),
        );
      }
    }
      ...
      listen<T>(_: unknown, event: string, arg: any): Event<T> {
        ...
        if (typeof target === 'function') {
          ...
          if (propertyIsEvent(event)) {
            mapEventNameToEvent.set(
              event,
              Event.buffer(handler[event] as Event<unknown>, true, undefined, disposables),
            );
