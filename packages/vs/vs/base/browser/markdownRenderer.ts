...
function sanitizeRenderedMarkdown(options: IInternalSanitizerOptions, renderedMarkdown: string): TrustedHTML {
...
export function renderMarkdownAsPlaintext(markdown: IMarkdownString, withCodeBlocks?: boolean) {
  ...
  const html = marked
    .parse(value, { renderer: withCodeBlocks ? plainTextWithCodeBlocksRenderer.value : plainTextRenderer.value })
