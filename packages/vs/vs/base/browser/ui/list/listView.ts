...
export class ListView<T> implements IListView<T> {
  ...
  constructor(
    ...
    this.disposables.add(
      addDisposableListener(this.rowsContainer, TouchEventType.Change, e => this.onTouchChange(e as GestureEvent)),
    );
  ...
  @memoize get onContextMenu(): Event<IListMouseEvent<T> | IListGestureEvent<T>> {
    return Event.any<IListMouseEvent<any> | IListGestureEvent<any>>(
      ...
      Event.map(
        this.disposables.add(new DomEmitter(this.domNode, TouchEventType.Contextmenu)).event as Event<GestureEvent>,
  ...
  private onDragOver(event: IListDragEvent<T>): boolean {
    ...
    event.browserEvent.dataTransfer.dropEffect =
      typeof result !== 'boolean' && result.effect?.type === ListDragOverEffectType.Copy ? 'copy' : 'move';
