...
import {
  ...
  MENU_MNEMONIC_REGEX,
  VerticalDirection,
} from 'vs/base/browser/ui/menu/menu';
...
export class MenuBar extends Disposable {
  ...
  constructor(
    ...
    this._register(
      DOM.addDisposableListener(this.container, DOM.EventType.FOCUS_OUT, e => {
        const event = e as FocusEvent;
  ...
  push(arg: MenuBarMenu | MenuBarMenu[]): void {
    ...
    menus.forEach(menuBarMenu => {
      ...
      if (this.isCompact) {
        ...
        this._register(
          DOM.addDisposableListener(buttonElement, EventType.Tap, (e: GestureEvent) => {
            // Ignore this touch if the menu is touched
            if (
              this.isOpen &&
              this.focusedMenu &&
              this.focusedMenu.holder &&
              DOM.isAncestor(e.initialTarget as HTMLElement, this.focusedMenu.holder)
            ) {
              return;
            }
        ...
        this._register(
          DOM.addDisposableListener(buttonElement, DOM.EventType.MOUSE_DOWN, (e: MouseEvent) => {
        ...
        this._register(
          DOM.addDisposableListener(buttonElement, DOM.EventType.MOUSE_UP, e => {
        ...
        this._register(
          DOM.addDisposableListener(buttonElement, DOM.EventType.MOUSE_ENTER, () => {
  ...
  createOverflowMenu(): void {
    ...
    this._register(
      DOM.addDisposableListener(buttonElement, DOM.EventType.MOUSE_DOWN, e => {
        // Ignore non-left-click
        const mouseEvent = new StandardMouseEvent(DOM.getWindow(buttonElement), e);
        if (!mouseEvent.leftButton) {
          e.preventDefault();
  ...
  private updateOverflowAction(): void {
    ...
    const showableMenus = this.menus.filter(
      menu => menu.buttonElement !== undefined && menu.titleElement !== undefined,
    ) as (MenuBarMenuWithElements & { titleElement: HTMLElement; buttonElement: HTMLElement })[];
