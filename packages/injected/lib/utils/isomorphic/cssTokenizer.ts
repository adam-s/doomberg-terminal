...
export function tokenize(str1: string): CSSTokenInterface[] {
  ...
  const next = function (num?: number) {
    ...
    if (num > 3) throw new Error('Spec Error: no more than three codepoints of lookahead.');
  ...
  const consumeAnIdentlikeToken = function (): CSSTokenInterface {
    ...
    if (str.toLowerCase() === 'url' && next() === 0x28) {
      ...
      else if (whitespace(next()) && (next(2) === 0x22 || next(2) === 0x27)) return new FunctionToken(str);
