...
import { type CSSComplexSelectorList, InvalidSelectorError, parseCSS } from './cssParser';
...
export function parseSelector(selector: string): ParsedSelector {
  ...
  for (const part of parsedStrings.parts) {
    ...
    if (kNestedSelectorNames.has(part.name)) {
      ...
      if (
        lastFrameIndex !== -1 &&
        selectorPartsEqual(nested.body.parsed.parts.slice(0, lastFrameIndex + 1), parts.slice(0, lastFrameIndex + 1))
      ) {
...
export function visitAllSelectorParts(
  selector: ParsedSelector,
  visitor: (part: ParsedSelectorPart, nested: boolean) => void,
) {
