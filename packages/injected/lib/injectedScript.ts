...
interface WebKitLegacyDeviceMotionEvent extends DeviceMotionEvent {
  readonly initDeviceMotionEvent: (
    type: string,
    bubbles: boolean,
    cancelable: boolean,
...
export class InjectedScript {
  ...
  querySelectorAll(selector: ParsedSelector, root: Node): Element[] {
    ...
    if (
      ...
      selector.parts[0].name === 'css' &&
      selector.parts[0].source === ':scope'
    ) {
        ...
        if (optionToSelect.valueOrLabel !== undefined) {
          matches =
            ...
            (optionToSelect.valueOrLabel === option.value ||
              optionToSelect.valueOrLabel === option.label);
        }
  ...
  setupHitTargetInterceptor(
    ...
    const listener = (event: PointerEvent | MouseEvent | TouchEvent) => {
      ...
      const point =
        !!this.window.TouchEvent && event instanceof this.window.TouchEvent
          ? event.touches[0]
    ...
    }
    {
      ...
      if (expression === 'to.be.in.viewport') {
        ...
        return {
          ...
          matches: ratio > 0 && ratio > (options.expectedNumber ?? 0) - 1e-9,
        };
      }
    }
...

    {
      ...
      if (expression === 'to.have.attribute.value') {
        ...
        received = options.useInnerText
          ...
          : elementText(new Map(), element).full;
