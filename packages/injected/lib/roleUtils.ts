...
const kImplicitRoleByTagName: { [tagName: string]: (e: Element) => string | null } = {
  ...
  HTML: () => 'document',
  IMG: (e: Element) =>
    e.getAttribute('alt') === '' && !e.getAttribute('title') && !hasGlobalAriaAttribute(e) && !hasTabIndex(e)
      ? 'presentation'
      : 'img',
  INPUT: (e: Element) => {
    ...
    if (!isOwnLabel && !isOwnLabelledBy) {
      ...
      if (['combobox', 'listbox'].includes(role)) {
        ...
        if (tagName === 'SELECT') {
          ...
          const listbox =
            role === 'combobox' ? queryInAriaOwned(element, '*').find(e => getAriaRole(e) === 'listbox') : element;
          selectedOptions = listbox
            ? queryInAriaOwned(listbox, '[aria-selected="true"]').filter(e => getAriaRole(e) === 'option')
            : [];
        }
  ...
  if (!['presentation', 'none'].includes(role)) {
    ...
    if (!labelledBy && (tagName === 'TEXTAREA' || tagName === 'SELECT' || tagName === 'INPUT')) {
      ...
      const usePlaceholder =
        (tagName === 'INPUT' &&
          ['text', 'password', 'search', 'tel', 'email', 'url'].includes((element as HTMLInputElement).type)) ||
        tagName === 'TEXTAREA';
