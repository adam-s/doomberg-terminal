...
export class Highlight {
  ...
  runHighlightOnRaf(selector: ParsedSelector) {
    ...
    this.updateHighlight(
      ...
      { tooltipText: asLocator(this._language, stringifySelector(selector)) },
    );
    this._rafRequest = this._injectedScript.builtinRequestAnimationFrame(() =>
      this.runHighlightOnRaf(selector),
    );
  }
  ...
  private _innerUpdateHighlight(
    ...
    this._glassPaneElement.style.pointerEvents = options.tooltipListItemSelected
      ? 'initial'
      : 'none';
    ...
    for (let i = 0; i < elements.length; ++i) {
      ...
      if (options.tooltipList || options.tooltipText || options.tooltipFooter) {
        tooltipElement =
          this._injectedScript.document.createElement('x-pw-tooltip');
        ...
        for (let index = 0; index < lines.length; index++) {
          ...
          if (options.tooltipListItemSelected) {
            ...
            element.addEventListener('click', () =>
              options.tooltipListItemSelected?.(index),
            );
          }
        }
        if (options.tooltipFooter) {
          const footer = this._injectedScript.document.createElement(
            'x-pw-tooltip-footer',
          );
    ...
    for (const entry of this._highlightEntries) {
      ...
      const { anchorLeft, anchorTop } = this.tooltipPosition(
        entry.box,
        entry.tooltipElement,
      );
  ...
  private _highlightIsUpToDate(
    ...
    if (
      options.tooltipListItemSelected !==
      this._highlightOptions.tooltipListItemSelected
    ) {
      return false;
    }
    ...
    if (
      options.tooltipList?.length !== this._highlightOptions.tooltipList?.length
    ) {
      return false;
    }
