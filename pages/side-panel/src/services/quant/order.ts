...
import { StrategySettings } from './settings';
...
export class Order {
  ...
  public readonly settings: StrategySettings;
  ...
  private async executeBuyOrder(): Promise<void> {
    ...
    try {
      ...
      // Get options for the selected expiration and type
      ...
      if (!options || options.length === 0) {
        throw new Error(
          `No options available for ${this.symbol} with expiration ${expirationDate}`,
        );
      }

      // Filter for out-of-the-money options
      const outOfMoneyOptions =
        optionType === 'call'
          ? options.filter(option => parseFloat(option.strike_price) > currentPrice)
          : options.filter(option => parseFloat(option.strike_price) < currentPrice);

      if (outOfMoneyOptions.length === 0) {
        throw new Error(`No out-of-the-money options available for ${this.symbol}`);
      }

      // Sort by strike price (closest to current price first)
      outOfMoneyOptions.sort((a, b) =>
        optionType === 'call'
          ? parseFloat(a.strike_price) - parseFloat(b.strike_price)
          : parseFloat(b.strike_price) - parseFloat(a.strike_price),
      );

      // Select the closest out-of-the-money option
      const selectedOption = outOfMoneyOptions[0];

      // Get market data directly from the option chain
      const marketData = optionChain.marketData$.get();
      if (!marketData || !marketData.get(selectedOption.id)) {
        throw new Error(`No market data available for option ${selectedOption.id}`);
      }

      // Access the quote directly from market data
      const quote = marketData.get(selectedOption.id);
      ...
      console.log(`Purchased option ${selectedOption.id} at price $${askPrice}`);
