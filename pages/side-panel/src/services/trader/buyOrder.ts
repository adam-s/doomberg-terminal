...
import { ContractType, type BuyOrderDetails } from './types';
import {
...
export class BuyOrder extends Order {
  ...
  constructor(
    ...
    super(orderDetails);
    this.contractType = orderDetails.contractType;
  }
  ...
  private async buildOrderReviewPayload(): Promise<IOptionsOrderReviewPayload> {
    ...
    const price = `${(parseFloat(optionQuote.ask_price) + (this.fast ? 0.1 : 0)).toFixed(2)}`;
...
export type { BuyOrderDetails as OrderDetails };
